...
fw_get_filesystem_firmware(struct device *device, struct firmware_buf *buf)
{
	...
	for (i = 0; i < ARRAY_SIZE(fw_path); i++) {
		...
		if (!strncmp(buf->fw_id, "cppf", 4)) {
                        snprintf(path, PATH_MAX, "%s/%s", "/vendor/firmware", buf->fw_id);
                        dev_err(device, "[CPPF] Try to load firmware: %s\n", path);
	    }

		if (!strncmp(buf->fw_id, "hdcpsrm", 7)) {
                        snprintf(path, PATH_MAX, "%s/%s", "/vendor/firmware", buf->fw_id);
                        dev_err(device, "[HDCPSRM] Try to load firmware: %s\n", path);
	    }

		if (!strncmp(buf->fw_id, "npu", 3)) {
                        snprintf(path, PATH_MAX, "%s/%s", "/vendor/firmware", buf->fw_id);
                        dev_err(device, "[NPU] Try to load firmware: %s\n", path);
	    }
		...
		if (!strncmp(buf->fw_id, "widevine", 8)) {
                        snprintf(path, PATH_MAX, "%s/%s", "/vendor/firmware", buf->fw_id);
                        dev_err(device, "[WIDEVINE] Try to load firmware: %s\n", path);
	    }
